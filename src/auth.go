...
func AuthAccs() {
	...
	if len(AccountsVer) == 0 {
		fmt.Println("Unable to continue, you have no Accounts added.")
		os.Exit(0)
	}
	...
	if len(Con.Bearers) == 0 {
		...
		if len(AccountsVer) != 0 {
			...
			fmt.Println("Unable to find any usable Accounts.")
			os.Exit(0)
		}
	}
}
				...
				if len(Proxy.Proxys) > 0 && Con.UseProxyDuringAuth {
					...
					go func() {
						...
						default:
							...
							fmt.Printf("Account %v Bearer is nil or it cannot name change.. [%v]\n", acc[0], acc[1])
						}
						wg.Done()
					}()
				} else {
		...
		for _, Accs := range AccountsVer {
			for _, num := range Con.Bearers {
				if Accs == num.Email+":"+num.Password {
					Con.Bearers = append(Con.Bearers, num)
				}
			}
		}
	}
